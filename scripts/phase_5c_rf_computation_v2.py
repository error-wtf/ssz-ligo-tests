...
where M_seconds = M_solar * 4.92549095e-6
"""
import h5py
import numpy as np
import csv
...
def compute_f_gr_kerr_220(m_solar, chi):
    """
    Compute GR QNM frequency for l=m=2, n=0 mode.
    Formula: Berti, Cardoso, Will (2009) analytical fit
    """
    M_seconds = m_solar * M_sun_seconds
    
    # Dimensionless frequency (omega * M)
    # Fit coefficients from Berti et al. for (2,2,0) mode
...
# Compute f_GR from H1's M and chi
...
# Compute R_f = f_measured / f_GR (sample-wise!)
R_f_h1 = f_h1 / f_gr_h1
print(f"\nR_f (H1, sample-wise):")
...
# Compute f_GR from L1's M and chi
...
print(f"\n{'='*80}")
print("COMPARISON WITH SSZ PREDICTION")
print("="*80)
print(f"\nSSZ QNM-Prediction for R_f:")
print(f"  Expected Shift: +39% -> R_f ~ 1.39")
print(f"  Lower limit (typical): +10% -> R_f ~ 1.10")
print()
print(f"Measured R_f values:")
...
# Check against preregistered thresholds
print(f"Preregistered threshold check:")
print(f"  R_f < 1.10: SSZ falsified for this test")
print(f"  R_f > 1.10: SSZ not falsified")
print()
...
if h1_below and l1_below:
    print(f"\n  -> BOTH under 1.10: SSZ QNM-Shift NOT supported")
    print(f"  -> TEST RESULT: SSZ_FALSIFIED_BY_THIS_TEST")
else:
    print(f"\n  -> At least one above 1.10: No clear falsification")
